...
function footer() {
  return `<footer class="footer"><div class="wrap"><p><strong>BuildMyCVNow</strong> — Free CV Builder for UAE, GCC & Philippines Job Seekers</p><p><a href="/about">About</a> · <a href="/privacy">Privacy</a> · <a href="/terms">Terms</a> · <a href="/faq">FAQ</a> · <a href="/blog">Blog</a></p><p>© 2026 BuildMyCVNow. All rights reserved.</p></div></footer>`;
}

function ctaCard(text = "Build your free CV") {
  return `<div class="card" style="background:#ecfdf5;border-color:#bbf7d0"><h3>${text}</h3><p>Create an ATS-friendly CV for UAE and Gulf jobs in minutes. No sign-up required.</p><a class="cta" href="/builder">Build My CV Now — Free</a></div>`;
}
...
writePage("/", layout({
  title: "Free CV Builder for UAE & Gulf Jobs | BuildMyCVNow",
  description: "Build a professional, ATS-friendly CV in 5 minutes. Free templates for Dubai, Abu Dhabi and GCC jobs. No sign-up needed.",
  canonical: baseUrl,
  jsonLd: [webAppSchema, faqSchema, orgSchema],
  body: `<main><section class="hero"><div class="wrap hero-grid"><div><h1>Free CV Builder for UAE & Gulf Jobs — ATS-Friendly, No Sign-Up</h1><p class="lead">Build a professional CV in 5 minutes. Templates designed for jobs in Dubai, Abu Dhabi, and across the GCC. 100% free — no hidden paywall, no watermark.</p><p><a class="cta" href="/builder">Build My CV Now — Free</a> <a class="btn secondary" href="/cv-examples">See CV examples</a></p><p class="trust">No sign-up required · ATS-friendly · Free PDF download</p></div><div class="template-thumb"><h3>Finished UAE CV preview</h3><p class="muted">Professional Summary</p><p>Reliable applicant with UAE-ready format, visa status, skills, and work experience.</p></div></div></section><div class="proof">Trusted by job seekers across the UAE, Saudi Arabia, Qatar & the Philippines · 12,000+ CVs started</div>${howItWorks()}${whySection()}${templatesPreview()}${faqSection()}<section class="section cta-band"><div class="wrap"><h2>Ready to build your CV?</h2><p>Free forever, no sign-up, and built for UAE/GCC applications.</p><a class="cta" href="/builder">Start free now</a></div></section></main>`,
}));

function howItWorks() {
  return `<section class="section"><div class="wrap"><h2>How it works</h2><div class="grid grid-3"><div class="card"><h3>1. Choose a template</h3><p>Pick a layout built for your industry. Hospitality, engineering, logistics, sales, finance, and skilled-worker formats are ready.</p></div><div class="card"><h3>2. Fill in your details</h3><p>Add your work history, visa status, languages, and skills with UAE-specific guidance beside the fields.</p></div><div class="card"><h3>3. Download your CV</h3><p>Check the live preview and ATS score, then download an ATS-ready PDF for free.</p></div></div></div></section>`;
}

function whySection() {
  const cards = [
    ["Built for the UAE market", "Add details Gulf recruiters often ask for, including visa status, nationality, languages, driving license, and photo conventions."],
  ...
  return `<section class="section"><div class="wrap"><h2>Why job seekers in the Gulf choose BuildMyCVNow</h2><div class="grid grid-4">${cards.map(([h, p]) => `<div class="card"><h3>${h}</h3><p>${p}</p></div>`).join("")}</div></div></section>`;
}

function templatesPreview() {
  return `<section class="section"><div class="wrap"><h2>CV templates for every industry</h2><div class="grid grid-4">${["Hospitality", "Engineering", "Finance", "Logistics"].map((item) => `<a class="card" href="/templates"><h3>${item}</h3><p class="muted">ATS-friendly UAE format with role-specific guidance.</p></a>`).join("")}</div></div></section>`;
}
...
writePage("/about", layout({ title: "About BuildMyCVNow | Free UAE CV Builder", description: "BuildMyCVNow helps UAE, GCC and Philippines job seekers create professional CVs for free.", canonical: `${baseUrl}/about`, body: `<main class="section"><div class="wrap"><h1>About BuildMyCVNow</h1><p class="lead">BuildMyCVNow was created to help job seekers build better CVs without paying before download.</p><p>We focus on UAE, GCC, and Philippines applicants who need clear, practical templates for real jobs. The mission is simple: make professional CV creation easy, mobile-friendly, and free.</p></div></main>` }));
...
writePage("/faq", layout({ title: "FAQ | Free CV Builder UAE", description: "Answers to common questions about free CV downloads, ATS-friendly CVs, UAE CV photos, visa status and mobile CV building.", canonical: `${baseUrl}/faq`, jsonLd: faqSchema, body: `<main>${faqSection()}</main>` }));
