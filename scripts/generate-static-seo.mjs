...
function staticHero() {
  const trust = ["No sign-up needed", "ATS-optimized", "5-minute build", "60+ countries"];
  const cards = [
  ...
  return `<section class="static-hero"><div class="wrap static-hero-grid"><div><div class="static-eyebrow">Free CV Builder - Works Worldwide</div><h1 class="static-headline">Every career.<br>Every country.</h1><p class="static-subhead">Build a professional, ATS-friendly CV in under 5 minutes. Free templates trusted by job seekers in the UAE, UK, US, Canada, Australia, and 60+ more countries.</p><div class="static-actions"><a class="cta" href="/builder">Build my CV - it's free</a><a class="btn secondary" href="#templates">Browse templates</a></div><div class="static-trust">${trust.map((item) => `<span>${item}</span>`).join("")}</div></div><div class="static-cv-stage" aria-label="Sample CV previews">${cards.map(([name, role, skills, color]) => `<article class="static-cv-card"><div class="static-cv-top" style="background:${color}"><strong>${name}</strong><span>${role}</span></div><div class="static-cv-body"><span class="static-badge">ATS-ready</span><p><strong>Professional Summary</strong></p><p>Clear, recruiter-friendly CV layout with global job details.</p><div class="static-line"></div><div class="static-line" style="width:80%"></div><p class="muted">${skills}</p></div></article>`).join("")}</div></div></section>`;
}
...
function staticVideoSection() {
  ...
  const buildSteps = ["Choose a template", "Upload your existing CV (optional)", "Fill in your personal information", "Add work experience and education", "Review your CV strength score", "Download your CV as PDF"];
  return `<section class="section static-video" id="how-it-works"><div class="wrap static-video-grid"><div><div class="static-browser"><div class="static-browser-bar"><span>BuildMyCVNow demo</span><a class="btn secondary" href="https://app.heygen.com/videos/buildmycvnow-landscape-boost-37034e74791e4040830908c8fe32f8a0" rel="noopener">Open video</a></div><div class="static-video-box"><div><h2 style="color:#fff;margin-bottom:10px">Watch how to create and download your CV for free</h2><p>2 min demo - no audio needed</p><p><a href="https://app.heygen.com/videos/buildmycvnow-landscape-boost-37034e74791e4040830908c8fe32f8a0" rel="noopener">Open the video walkthrough</a></p></div></div></div><div class="static-pills">${steps.map((step, index) => `<span class="static-pill">${index + 1}. ${step}</span>`).join("")}</div><div class="static-chapters">${chapters.map((chapter) => `<span class="chip">${chapter}</span>`).join("")}</div></div><div><div class="static-section-title" style="text-align:left;margin:0 0 22px"><h2>From blank page to polished CV in minutes</h2><p class="lead">Watch how easy it is - no design skills needed.</p></div><div class="static-step-list">${buildSteps.map((step, index) => `<div class="static-step"><strong>Step ${index + 1}</strong><br>${step}</div>`).join("")}</div><p class="muted"><strong>Tip:</strong> You can switch templates at any time without losing your data.</p><div class="static-stat-row"><div class="static-stat"><strong>5 min</strong>Average build</div><div class="static-stat"><strong>8+</strong>Job categories</div><div class="static-stat"><strong>100%</strong>Free PDF</div></div></div></div></section>`;
}
...
function staticFeaturesSection() {
  const features = [
    ["AI", "AI writing assist", "Check spelling, grammar, and rewrite rough duties into clear professional wording before you approve it."],
    ["CV", "Live preview", "See your CV update as you type, with a strength score to guide what still needs attention."],
    ["QR", "Share via link or QR", "Export and share your finished CV in ways that work for recruiters and mobile users."],
    ["GL", "Global-ready fields", "Add languages, visa status, nationality, driving license, LinkedIn, salary, and references only when needed."],
  ];
  return `<section class="section static-features"><div class="wrap"><div class="static-section-title"><h2>Everything you need, nothing you don't</h2><p class="lead">Simple tools for first-time applicants and experienced workers.</p></div><div class="grid grid-4">${features.map(([icon, title, text]) => `<article class="card"><div class="static-feature-icon">${icon}</div><h3>${title}</h3><p>${text}</p></article>`).join("")}</div></div></section>`;
}

function staticTestimonialsSection() {
  const testimonials = [
    ["Maria, Hotel Receptionist", "I made a clean hotel CV on my phone before applying. The wording helped me explain my front desk experience clearly."],
    ["Raj, IT Support", "The template looked professional and the ATS tips helped me keep my skills easy to scan."],
    ["Ana, Admin Assistant", "I liked that I could download without signing up. It was fast and simple."],
  ];
  return `<section class="section static-testimonials"><div class="wrap"><div class="static-section-title"><h2>Job seekers who got hired</h2><p class="lead">Short, practical CV help for real applications.</p></div><div class="grid grid-3">${testimonials.map(([name, quote]) => `<article class="card"><p class="static-quote">"${quote}"</p><p><strong>${name}</strong></p></article>`).join("")}</div></div></section>`;
}
...
writePage("/contact", layout({ title: "Contact BuildMyCVNow | CV Builder Support", description: "Contact BuildMyCVNow with questions, support requests, feedback, or partnership messages.", canonical: `${baseUrl}/contact`, body: `<main class="section"><div class="wrap"><h1>Contact BuildMyCVNow</h1><p class="lead">Have a question, support request, or suggestion? Send a message and the BuildMyCVNow team will review it.</p><form class="card" method="post" action="/contact" style="display:grid;gap:14px;max-width:680px"><label><strong>Name</strong><br><input name="name" required style="width:100%;padding:12px;border:1px solid #cbd5e1;border-radius:8px"></label><label><strong>Email</strong><br><input name="email" type="email" required style="width:100%;padding:12px;border:1px solid #cbd5e1;border-radius:8px"></label><label><strong>Message</strong><br><textarea name="message" required rows="6" style="width:100%;padding:12px;border:1px solid #cbd5e1;border-radius:8px"></textarea></label><p><a class="cta" href="/builder">Open CV builder</a></p></form></div></main>` }));
const ofwFaqs = [
