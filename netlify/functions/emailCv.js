...
const sendEmail = async ({ toEmail, cv }) => {
  ...
  if (!response.ok) {
    ...
    const message = isServerApiBlocked
...
export const handler = async (event) => {
  ...
  let forwarded = false;
  try {
    forwarded = await sendEmail({ toEmail, cv: payload.cv });
  } catch (error) {
    return json(502, {
      ok: false,
      forwarded: false,
      providerCode: error.providerCode,
      message: error.message || "Could not send the CV email. Please download the file and try again later.",
    });
  }
  return json(200, {
    ok: true,
