...
const listToText = (value, separator = "\n") => {
  if (Array.isArray(value)) return value.map(valueToText).filter(Boolean).join(separator);
...
const normalizeCv = (data = {}, fileName = "") => {
  ...
  return {
    ...
    education: listToText(data.education),
    certifications: listToText(data.certifications),
    languages: listToText(data.languages, ", "),
    references: valueToText(data.references) || "Available upon request",
